"""
ensemble.py — UC200 (bird image classification) ensemble orchestrator.

Implements the 4-round KF pipeline for fine-grained visual species classification.
...
async def run_ensemble(
    ...
    verbose: bool = True,
    dataset: str = "bird-uc200",
    dataset_tag: str = DATASET_TAG,
    max_revisions: int = MAX_REVISIONS,
    test_mode: bool = False,
) -> dict:
    """Run the full UC200 ensemble on a single image classification task.

    Args:
        task: Dict with keys:
            pair_id          str   e.g. "american_crow_vs_fish_crow"
            class_a          str   e.g. "American Crow"
            class_b          str   e.g. "Fish Crow"
            class_id_a       int
            class_id_b       int
            test_image_path  str   absolute path to the test image
            test_label       str   ground truth species name
            few_shot_a       list[str]  paths to labeled class_a images (for verifier)
            few_shot_b       list[str]  paths to labeled class_b images
        task_id: Unique identifier for this image (e.g. "american_crow_vs_fish_crow_00042")
        ...
        dataset_tag: Namespace tag ("bird-uc200").
    ...
    # ------------------------------------------------------------------
    log("Round 1: OBSERVER filling feature schema from image...")
    t_r1 = time.time()
    ...
    if not test_mode and correct_label:
        ...
        # Extract new visual rules from this outcome
